...
def main():
    ...
    out_dir = '/media/shuoz/MyDrive/HCP/%s/Results/Fix_OC' % atlas
    ...
    idx_male = np.where(genders['REST1']['gender'] == 0)[0]
    idx_female = np.where(genders['REST1']['gender'] == 1)[0]

    x, y, x1, y1 = _pick_half(data['REST1'], random_state=2021)

    xy_fix = {0: (x[idx_male], y[idx_male], genders['REST1']['gender'][idx_male]),
              1: (x[idx_female], y[idx_female], genders['REST1']['gender'][idx_female])}
        ...
        for i_split in range(5):
            ...
            for i_fold in [0, 1]:
                ...
                for train_gender in [0, 1]:
                    x_fix, y_fix, g_fix = xy_fix[1 - train_gender]
                    x_fix = torch.from_numpy(x_fix)
                    x_fix = x_fix.float()
                    # y_fix = torch.from_numpy(y_fix)
                    # y_fix = y_fix.long()
                    g_fix = torch.from_numpy(g_fix.reshape((-1, 1)))
                    g_fix = g_fix.float()
    ...
    out_file = os.path.join(out_dir, 'results_sub_half_fix_oc.csv')
