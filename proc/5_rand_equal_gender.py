...
def main():
    ...
    np.random.seed(144)
    idx_female_rand = np.random.choice(idx_female, size=idx_male.shape[0], replace=False)
    ...
    for session in sessions:
        ...
        out_fname = 'HCP_%s_%s_half_brain_%s_gender_equal_%s.hdf5' % (atlas, connection_type, session, run_)
        io_.save_half_brain(out_dir, out_fname, data[session]["Left"], data[session]["Right"])
