...
def run_experiment(cfg):
    ...
    out_filename = "%s_results_%s_L%s_test_size_%s_%s" % (
        dataset,
        int(lambda_),
...
def save_loop_results(
    model, res_dict, xy_test, train_group, train_session, lambda_, i_split, i_fold
):
...
def run_no_sub_hold_hcp(
    data, groups, lambda_, l2_param, mix_group, out_dir, num_repeat, random_state
):
    # main loop
    res = {
        **{
        ...
        for i_split in range(num_repeat):
            ...
            for i_fold in [0, 1]:
                ...
                for train_group in [0, 1]:
                    ...
                    model_filename = (
                        "HCP_lambda%s_%s_%s_%s_gender_%s_%s_test_size00"
                        % (
                            int(lambda_),
                            train_session,
                            i_split,
        ...
        for i_split, (train_sub, test_sub) in enumerate(sss.split(x, groups)):
            for train_fold in [0, 1]:
                ...
                for target_group in [0, 1]:
                    ...
                    model_filename = "HCP_lambda%s_%s_%s_gender_%s_%s_test_size_0%s" % (
                        int(lambda_),
                        i_split,
                        train_fold,
                        target_group,
