...
def constructW(
    ...
    if NeighborMode_low == "KNN".lower():
        ...
        assert gnd is not None, (
            "Label(gnd) should be provided under " "Supervised" " NeighborMode!"
        )
    ...
    if NeighborMode_low == "Supervised".lower():
        ...
        if WeightMode_low == "Binary".lower():
            ...
            if k > 0:
                ...
                for i in range(nLabel):
                    classIdx = np.where(gnd == Label[i])
                    G[np.ix_(classIdx, classIdx)] = np.dot(
                        fea[classIdx, :], fea[classIdx, :].T
                    )
            if not bSelfConnected:
                if isinstance(G, np.ndarray):
                    np.fill_diagonal(G, 0)
    ...
    if (NeighborMode.lower() == "KNN".lower()) and (k > 0):
        if not (bCosine and bNormalized):
            G = np.zeros([nSmp * (k + 1), 3])
            for i in range(int(np.ceil(float(nSmp) / BlockSize))):
                if i == int(np.ceil(float(nSmp) / BlockSize)) - 1:
                    ...
                    if not bBinary:
                        if bCosine:
                            ...
                            dumpInd = sub2ind(
                                dist.shape, [linidx[:, tuple([1] * idx.shape[1]), idx]]
                            )
                            dump = dist[dumpInd]
                        else:
                            dump = np.exp(-dump / (2 * (t ** 2)))
                    G[i * BlockSize * (k + 1) : nSmp * (k + 1), 0] = np.array(
                        list(smpIdx) * (k + 1)
                    )
                    ...
                    if not bBinary:
                        if bCosine:
                            ...
                            linidx = range(idx.shape[0])
                            dumpInd = sub2ind(
                                dist.shape, [np.tile(linidix, [1, idx.shape[1]]), idx]
                            )
                            dump = dist[dumpInd]
                        else:
                            dump = np.exp(-dump / (2 * (t ** 2)))
                    G[
                        i * BlockSize * (k + 1) : (i + 1) * BlockSize * (k + 1), 0
                    ] = np.tile(
                        smpIdx, [k + 1, 1]
                    )  ##################
                    G[i * BlockSize * (k + 1) : (i + 1) * BlockSize * (k + 1), 1] = vec(
                        idx
                    )
                    if not bBinary:
                        G[
                            i * BlockSize * (k + 1) : (i + 1) * BlockSize * (k + 1), 2
                        ] = vec(dump)
                    else:
                        G[
                            i * BlockSize * (k + 1) : (i + 1) * BlockSize * (k + 1), 2
            ...
            for i in range(int(np.ceil(float(nSmp) / BlockSize))):
                if i == int(np.ceil(float(nSmp) / BlockSize)) - 1:
                    ...
                    G[
                        i * BlockSize * (k + 1) : (i + 1) * BlockSize * (k + 1), 0
                    ] = np.tile(smpIdx, [1, k + 1])
                    G[i * BlockSize * (k + 1) : (i + 1) * BlockSize * (k + 1), 1] = vec(
                        idx
                    )
                    G[i * BlockSize * (k + 1) : (i + 1) * BlockSize * (k + 1), 2] = vec(
                        dump
                    )
            W = scipy.sparse.coo_matrix((G[:, 0], [G[:, 1], G[:, 2]]), (nSmp, nSmp))
            W = W.tolil()
