...
def JIVE(Datatypes_in, r, rIndiv, scale=True, ConvergeThresh=1e-10, MaxIter=1000):
    ...
    assert isinstance(Datatypes, (tuple, list)), (
        "Error: Datatypes should be "
    ...
    for j in range(MaxIter):
        ...
        if np.linalg.norm(X_est - (J + A)) ** 2.0 < ConvergeThresh:
            break
        X_est = J + A
        if j == MaxIter:
...
def JIVE_RankSelect(Datatypes, alpha, Nperm, scale=True, convThresh=1e-8, MaxIter=1000):
    ...
    """
    assert isinstance(
        Datatypes, (tuple, list)
    ), "Error: Datatypes should be either list or tuple"
    nDatatypes = len(Datatypes)
    d = [None] * nDatatypes
    n = [None] * nDatatypes
    for i in range(nDatatypes):
    ...
    while (r != rPrev) or (not np.all(rIndivPrev == rIndiv)):
        ...
        for k in range(nDatatypes):
            ...
            PermThreshI[k] = np.percentile(
                SingValsPermI[k], (1 - alpha) * 100, interpolation="midpoint", axis=0
            )
            rIndiv[k] = 0
            for i in range(min(d[k], n)):
        ...
        PermThresh = np.percentile(
            SingValsPerm, (1 - alpha) * 100, interpolation="midpoint", axis=0
        )
        r = 0
        for i in range(min(sum(d), n)):
