...
def mmc_nonnegative(A, Qini, iters=1000, tol=1e-8):
    ...
    for it in range(iters):
        dA = (1.0 + eps) / (np.diag(np.dot(Q.T, np.dot(A, Q))) + eps)  #
        tmp1 = Q.T * D  #
        QQ = np.dot(tmp1, Q)
        dD = np.diag(QQ)
        Qb = Q * (dA ** 2.0)
        ...
        Q = Q * S
        tmp = Q.T * D
        Q = Q * np.sqrt((1.0 + eps) / (np.diag(np.dot(tmp, Q)) + eps))
        QQI = QQ - np.eye(class_num)
        obj[it] = sum(dA) - np.trace(np.dot(Lambda, QQI))
        obj1[it] = sum(dD * dA)
        orobj[it] = np.sqrt(
            np.trace(np.dot(QQI.T, QQI) / (class_num * (class_num - 1)))
        )  #### or class_num*(class_num+1) ?
