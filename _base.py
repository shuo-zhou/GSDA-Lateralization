...
def param_search(x, y, clf_="SVC", co_variates=None, reduce_dim=True):
    ...
    param_grids = {
        "PCA": {"n_components": [500, 300, 200, 100]},
        "SVC": {"C": np.logspace(-4, 3, 8)},
        "Logistic": {"C": np.logspace(-4, 3, 8)},
        "Ridge": {"eig_vectors": np.logspace(-4, 3, 8)},
        "SIDeRSVM": {"C": np.logspace(-4, 3, 8), "lambda_": np.logspace(-4, 3, 8)},
        "SIDeRLS": {"sigma_": np.logspace(-4, 3, 8), "lambda_": np.logspace(-4, 3, 8)},
    }
    default_params = {
        "SVC": {"C": 1.0, "max_iter": 1000000},
        "Logistic": {"C": 1.0},
    ...
    if reduce_dim:
        ...
        for n_comp in param_grids["PCA"]["n_components"]:
            ...
            if best_acc == 1.0:
                best_estimator = {"PCA": pca, "clf": clf_default}
    ...
    for param in param_grid:
        kwd_params = {
            key: best_params["clf"][key] for key in best_params["clf"] if key != param
        }
        best_acc = 0
