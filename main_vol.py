...
def main():
    ...
    genders = info['gender'].values.reshape((-1, 1))
    data = io_.load_half_brain(data_dir, atlas, data_type=cfg.DATASET.TYPE)

    res = {"acc_ic": [], "acc_oc": [], 'pred_loss': [], 'code_loss': [], 'lambda': [], 'split': [], 'fold': [],
           'train_gender': [], 'time_used': []}
    ...
    for i_split in range(num_repeat):
        ...
        for i_fold in [0, 1]:
            ...
            for train_gender in [0, 1]:
                ...
                model_path = os.path.join(out_dir, "volume_lambda_%s_%s_%s_gender_%s_%s.pt" %
                                          (lambda_, i_split, i_fold, train_gender, random_state))
                if os.path.exists(model_path):
                    model = torch.load(model_path)
                else:
                    model.fit(x_train, y_train[ic_idx], genders, target_idx=ic_idx)
    ...
    out_file = os.path.join(out_dir, 'volume_results_lambda_%s_sub_half_%s_%s.csv' % (lambda_, run_, random_state))
